...
custom_list="/var/www/html/pihole-cat/manage/categories/custom.list"
adlists_list="/etc/pihole/adlists.list"

pihole_update(){
		#sudo /usr/bin/cat ${custom_list} > ${adlists_list}
		/usr/bin/cat /var/www/html/pihole-cat/manage/categories/custom.list > /etc/pihole/adlists.list 
